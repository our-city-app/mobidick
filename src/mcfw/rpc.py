...
class MissingArgumentException(Exception):
    ...
    def __init__(self, name, func=None):
        Exception.__init__(self, "%s is a required argument%s!" % (name, (' in function %s' % func.func_name) if func else ''))
        self.name = name
        ...
        def logged(*args, **kwargs):
            ...
            try:
                ...
                if response:
                    ...
                    logging.debug(u"%s.%s finished in %s seconds returning %s" % (f.__module__, f.__name__, end - start, result))
                return result
            except:
                if response:
                    end = time.time()
...
def arguments(**kwarg_types):
    ...
    def wrap(f):
        ...
        f_arg_defaults = dict((f_args[0][i], f_defaults[i - f_arg_no_defaults_count] if i >= f_arg_no_defaults_count else MISSING) for i in xrange(f_arg_count))
        f_pure_default_args_dict = dict((f_args[0][i], f_defaults[i - f_arg_no_defaults_count]) for i in xrange(f_arg_no_defaults_count, f_arg_count))
        ...
        def typechecked_f(*args, **kwargs):
            ...
            map(lambda arg: _check_type(arg, kwarg_types[arg], kwargs[arg], accept_missing=accept_missing, func=f), kwargs)
            return f(**kwargs)
...
def get_parameters(parameters, kwarg_types):
    return dict(map(lambda (name, type_):(name, parse_parameter(name, type_, parameters[name]) if name in parameters else MISSING), kwarg_types.iteritems()))
    ...
    if isinstance(checktype, list) and isinstance(value, list):
        ...
        for i, x in enumerate(value):
            ...
            if not isinstance(x, t):
                raise ValueError("%s: Not all items were of expected type %s. Encountered an item at index %s with type %s: %s."
...
def _get_complex_serializer(type_):
    if not type_ in _complex_serializer_cache:
        def serializer(value, skip_missing):
            ...
            result = dict([(name, getattr(value, name)) for (name, _) in simple_members if not skip_missing or getattr(value, name) != MISSING])
            ...
            result.update(dict([_serialize(name, prop) for (name, prop) in complex_members if not skip_missing or getattr(value, name) != MISSING]))
