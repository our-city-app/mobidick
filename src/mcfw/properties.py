...
try:
    ...
    pass  # Allow running outside app engine
...
class typed_property(object):
    ...
    def __init__(self, name, type_, list_=False, doc=None, subtype_attr_name=None, subtype_mapping=None, default=MISSING, hash_serializer=str):
        self.type = type_
        self.list = list_
        ...
        if value != MISSING:
            if self.list:
                if not isinstance(value, (list, tuple, set)):
                    raise ValueError("Expected [%s] for '%s' and got %s - %s!" % (self.type, self.attr_name, type(value), value))
...
class unicode_property(typed_property):
    ...
    def __init__(self, name, doc=None, empty_string_is_null=False, default=MISSING):
        typed_property.__init__(self, name, unicode, False, doc, default=default, hash_serializer=lambda x: __none__ if x is None else x.encode('utf8'))
